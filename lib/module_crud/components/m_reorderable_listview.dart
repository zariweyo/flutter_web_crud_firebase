...
/// The callback used by [MReorderableListView] to move an item to a new
/// position in a list.
///
...
/// [MReorderableListView] will need to account for this when inserting before
/// [newIndex].
///
...
/// ```
/// {@end-tool}
typedef MReorderCallback = void Function(int oldIndex, int newIndex);
typedef MOffsetCallback = void Function(int index,Offset offset);
typedef MStartDragCallback = void Function();
...
class MReorderableListView extends StatefulWidget {

  /// Creates a reorderable list.
  MReorderableListView({
    this.header,
  ...
  /// underlying list.
  ///
  /// This [MReorderableListView] calls [onReorder] after a list child is dropped
  /// into a new position.
  final MReorderCallback onReorder;

  //DCC Extends
  final MOffsetCallback onOffsetOut;
  final Color colorOnDrag;
  final MStartDragCallback onStartDrag;
  final Axis dragDirection;
  final double  sizeHeader;
  final bool  canDrag;

  @override
  _MReorderableListViewState createState() => _MReorderableListViewState();
}
...
class _MReorderableListViewState extends State<MReorderableListView> {
  // We use an inner overlay so that the dragging list item doesn't draw outside of the list itself.
  final GlobalKey _overlayKey = GlobalKey(debugLabel: '$MReorderableListView overlay key');
    ...
    _listOverlayEntry = OverlayEntry(
      ...
      builder: (BuildContext context) {
        return _ReorderableListContent(
          ...
          onOffsetOut: widget.onOffsetOut,
          colorOnDrag: widget.colorOnDrag,
          onStartDrag: widget.onStartDrag,
          dragDirection: widget.dragDirection,
          sizeHeader: widget.sizeHeader,
...
class _ReorderableListContent extends StatefulWidget {
  const _ReorderableListContent({
    ...
    @required this.onReorder,
    @required this.onOffsetOut,
    @required this.reverse,
    @required this.colorOnDrag,
...
class _ReorderableListContentState extends State<_ReorderableListContent> with TickerProviderStateMixin<_ReorderableListContent> {
  ...
  //List<Widget> _currentChildren = new List<Widget>();
  ...
  void initState() {
    ...
    //DCC added for construct de scrollController independent 
    _scrollController = new ScrollController();
  }

  @override
  void didChangeDependencies() {
    //DCC If this, the scrollcontroller won't accept multi lists
  ...
  void _requestAnimationToNextIndex() {
    ...
    if (_entranceController.isCompleted) {
      //print("DCC -------------");
      //print("$_nextIndex == $_currentIndex == $_ghostIndex");
  ...
  // Animates the droppable space from _currentIndex to _nextIndex.
  void _requestAnimationToNextIndex_old() {
    
    if (_entranceController.isCompleted) {
      //print("$_nextIndex == $_currentIndex");
      _ghostIndex = _currentIndex;
      if (_nextIndex == _currentIndex) {
        return;
      }
...
/*
      Widget _widgetToMove = _currentChildren[startIndex];
      List<Widget> _newCurrentChildren = new List<Widget>();
      int _idx = 0;
      _currentChildren.forEach((_child){
        if(_child.key==_widgetToMove.key){
          print("LOCATED");
          return;
        }
        if(_idx==endIndex){
          //_newCurrentChildren.add(_widgetToMove);
          _idx++;
        }
        _newCurrentChildren.add(_child);
        _idx++;
      });

      if(_idx<_currentChildren.length){
        //_newCurrentChildren.add(_widgetToMove);
      }
*/ 

      setState(() {
 //       _currentChildren = _newCurrentChildren;

        if (startIndex != endIndex){

          //DCC on down move, alwais add one more
      ...
      if(widget.canDrag){
        child=Draggable<Key>(
          ...
          feedback: Container(
            ...
            child: Material(
              elevation: 10.0,
              color:widget.colorOnDrag==null?Colors.white:widget.colorOnDrag,
          ...
          onDraggableCanceled: (Velocity velocity, Offset offset) {
            //DCC Detect offset out for actions
            if(widget.onOffsetOut!=null){
              widget.onOffsetOut(_dragStartIndex,offset);
            }
            onDragEnded();
          },
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(builder: (BuildContext context, BoxConstraints constraints) {
      ...
      if(widget.sizeHeader != null){
        cards.add(
          SizedBox(height:widget.sizeHeader)
        );
      }
      for (int i = 0; i < widget.children.length; i += 1){

        cards.add(_wrap(widget.children[i], i, constraints));
      }
      if(!widget.reverse){
        cards.add(
          _wrap(finalDropArea, widget.children.length, constraints)
